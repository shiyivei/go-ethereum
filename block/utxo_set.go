...
func (utxoSet *UTXOSet) FindUTXOForAddress(address string) []*economic_model.UTXO {

	var utxos []*economic_model.UTXO
	...
	utxoSet.BlockChain.DB.View(func(tx *bolt.Tx) error {
		...
		for k, v := c.First(); k != nil; k, v = c.Next() {
			...
			txOutputs := economic_model.DeserializeTXOutputs(v)
...
func (utxoSet *UTXOSet) Update() {
	...
	txOutputsMap := make(map[string]*economic_model.TXOutputs)
	...
	utxoMap := make(map[string][]*economic_model.UTXO)
	if len(utxos) > 0 {
		for _, utxo := range utxos {
		...
		for k, v := range utxoMap {
			txOutputsMap[k] = &economic_model.TXOutputs{v}
	...
	err := utxoSet.BlockChain.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			for _, input := range txInputs {
				...
				txoputs := economic_model.DeserializeTXOutputs(txOutputsBytes)
				...
				for _, uos := range txoputs.UTXOS {
					...
					if input.Vout == uos.Index && bytes.Compare(uos.Output.Ripemd160Hash, wallet.Ripemd160Hash(input.PublicKey)) == 0 {
						...
						txOutputsMap[txHash] = &economic_model.TXOutputs{utxos}
					}
				}
		...
		uosMap := make(map[string][]*economic_model.UTXO)
		if len(utxos) > 0 {
