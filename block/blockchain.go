...
func CreatBlockchainWithGenesisBlock(address string, nodeID string) *Blockchain {
	...
	if DBExists(dbName) {
		fmt.Println("BlockChain has been created")
		os.Exit(1)
	}
...
func (blockchain *Blockchain) MineNewBlock(from, to, amount []string, nodeID string) {
	...
	err1 := blockchain.DB.Update(func(tx *bolt.Tx) error {
		...
		b := tx.Bucket([]byte(blockTableName))
		if b != nil {
			err2 := b.Put(block.Hash, Serialize(block))
			if err2 != nil {
...
func (blockchain *Blockchain) UnUTXOs(address string, txs []*transaction.Transaction) []*economic_model.UTXO {
	var unUTXOs []*economic_model.UTXO
		...
		for index, out := range tx.Vouts {
			if out.UnlockScriptPublicKeyWithAddress(address) {
				if len(spentTXOutputs) == 0 {
					utxo := &economic_model.UTXO{tx.TxHash, index, out}
...
func (blockchain *Blockchain) FindUTXOMap() map[string]*economic_model.TXOutputs {

	blcIterator := blockchain.Iterator()

	//collect all inputs
	spentableUTXOMap := make(map[string][]*economic_model.TXInput)
	utxoMaps := make(map[string]*economic_model.TXOutputs)
	...
	for {
		...
		for i := len(block.Txs) - 1; i >= 0; i-- {
			...
			txOutputs := &economic_model.TXOutputs{[]*economic_model.UTXO{}}
		...
		workOutLoop:
			...
			for index, output := range tx.Vouts {
				...
				if len(txInputs) > 0 {
					...
					if isSpent == false {
						utxo := &economic_model.UTXO{tx.TxHash, index, output}
...
func GobDecode(blockChainBytes []byte) *Blockchain {
	var blockChain Blockchain

	decoder := gob.NewDecoder(bytes.NewReader(blockChainBytes))
